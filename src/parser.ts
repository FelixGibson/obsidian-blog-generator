...
export function parse(
    ...
    for (let i = 0; i < lines.length; i++) {
        const indent = stack[stack.length - 1].indentOnCardSeparatorLineNumber;
        if (
            lines[i].length === 0 ||
            (stack.length > 0 &&
                indent !== null &&
                getIndent(lines[indent]) >
                    getIndent(lines[i]))
        ) {
            const cardType = stack[stack.length - 1].cardType;
            if (stack.length > 0 && cardType !== null) {
